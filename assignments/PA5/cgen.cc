...
void get_methods_recursively(Class_ cls, std::vector<std::pair<Class_, method_class *> > &methods)
{
    ...
    for (int i = features->first(); features->more(i); i = features->next(i)) {
        ...
        for (std::vector<std::pair<Class_, method_class *> >::iterator it = methods.begin(); it != methods.end(); it++) {
...
{
    for(std::vector<Class_>::iterator it_c = cls_ordered.begin(); it_c != cls_ordered.end(); it_c++) {
        Class_ cls = *it_c;
        str << cls->get_name() << DISPTAB_SUFFIX << LABEL;
        get_methods_recursively(cls, cls->all_methods);

        for (std::vector<std::pair<Class_, method_class *> >::iterator it_m = cls->all_methods.begin(); it_m != cls->all_methods.end(); it_m++) {
...
void CgenClassTable::code_prototypes()
{
    for(std::vector<Class_>::size_type i = 0; i < cls_ordered.size(); i++) {
        ...
        for (std::vector<attr_class *>::size_type i = 0; i < cls->all_attrs.size(); i++) {
			attr_class * attr = cls->all_attrs[i];
